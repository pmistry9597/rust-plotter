...
use super::{store::{Store, StoreFns}, change_desrip::ChangeDescrip, processor::Processor, notify_hook::NotifyHook};
        ...
        Notifier: NotifyHook,
{
    ...
    pub fn change(self: &mut Self, raw: &'r Store<RawType, RawStore>, change: &ChangeDescrip) -> ChangeDescrip {
        let out_descrip = self.processor.change(raw, &mut self.out, change);
